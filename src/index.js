...
socket.onmessage = function(message){
  ...
  if(data.event == "login"){
    ...
    new CollectionView({
      ...
      createCell: () => new Composite({highlightOnTouch: true})
      .append( new TextView({ centerX:0, centerY:0, textColor:'white'})),
  
      updateCell: (cell, index) =>  {
        const item = friendsArray[index];
        cell.find(TextView).only().text = item.username;
...
function appendChat(){
  ...
  new NavigationView({layoutData:'stretch',drawerActionVisible: true, toolbarColor: headerColor})
  .append(new Page({title: 'Chat' ,background: backgroundColor, autoDispose: true}))
  .appendTo(contentView);
...
function appendProfile(){
  drawer.close()
  new NavigationView({layoutData:'stretch',drawerActionVisible: true, toolbarColor: headerColor})
  .append(new Page({title: 'Profile' ,background: backgroundColor, autoDispose: true}))
  .appendTo(contentView);
}
...
function appendFriends(){
  drawer.close()
  new NavigationView({layoutData:'stretch',drawerActionVisible: true, toolbarColor: headerColor})
  .append(new Page({title: 'Friends' ,background: backgroundColor, autoDispose: true}))
  .appendTo(contentView);

  const friendCode = new TextInput({
    top: 70, left: '10', right: '10',borderColor:blueColor, messageColor:blueColor,textColor:blueColor,
    message: 'Add Friend'
  }).appendTo(contentView).on('accept',function(ev){
...
function appendLogin(){
  ...
  new NavigationView({layoutData: 'stretch', toolbarColor: headerColor})
  .append(new Page({title: 'Log In' ,background: backgroundColor, autoDispose: true}))
  .appendTo(contentView);

  const loginComposite = new Composite({
    centerY: 0,
    centerX: 0,
    width: 300,
  }).appendTo(contentView);

  const char1 = new TextInput({
    alignment: 'centerX',
    width: 50,
    centerX: -120,
    keyboard: 'number',
...
function authenticated(uid){
  ...
  new CollectionView({
    ...
    createCell: () => new Composite({highlightOnTouch: true})
    .append(new ImageView({left: 16, width: 24, height: 24, centerY: 0}), new TextView({ left: 72, centerY:0})),

    updateCell: (cell, index) =>  {
      const item = items[index];
      cell.find(TextView).only().text = item ? item.title : pages[index].title;
      cell.on('tap', item.onclick);
      cell.find(ImageView).only().image = item ? item.image : '/resources/png/baseline_chat_black_18dp.png';
    }
  }).appendTo(drawer);
